...
L4RE_ELF_AUX_ELEM_T(l4re_elf_aux_mword_t, __stack_addr,
		L4RE_ELF_AUX_T_STACK_ADDR, 0xb1000000);
...
static void insert_regions()
{
	...
	while ((n = L4Re::Env::env()->rm()->get_regions(addr, &rl)) > 0)
	{
		for (int i = 0; i < n; ++i)
		{
			...
			void *x = __local_rm.attach((void*) r->start, r->end - r->start + 1,
					Region_handler(
							L4::cap_reinterpret_cast<L4Re::Dataspace>(
									Env::env()->rm()), L4_INVALID_CAP, 0,
							Rm::Pager), 0);
			if (!x)
			{
	...
	while ((n = L4Re::Env::env()->rm()->get_areas(addr, &al)) > 0)
	{
		for (int i = 0; i < n; ++i)
		{
			...
			l4_addr_t x = __local_rm.attach_area(r->start,
					r->end - r->start + 1);
			if (!x)
			{
				L4::cerr << "l4re: error while initializing region mapper\n";
...

		{
			...
			if (!obj.is_valid())
			{
				Err(Err::Fatal).printf(
						"could not find the base memory allocator\n");
				return -1;
			}
